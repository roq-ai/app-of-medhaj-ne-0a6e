...
import { ContributorListPage as ShowCaseList } from 'pages/contributors';

function DashboardPage() {
  const { session } = useSession();

  const articleStat = useSWR<PaginatedInterface<ArticleInterface>>(
    () => `/articles`,
    () => getArticles({ limit: 1, offset: 0 }),
  );
  ...
  const contributorStat = useSWR<PaginatedInterface<ContributorInterface>>(
    () => `/contributors`,
    () => getContributors({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Articles',
      result: articleStat?.data?.totalCount,
      isLoading: articleStat.isLoading,
    ...
    {
      title: 'Contributors',
      result: contributorStat?.data?.totalCount,
      isLoading: contributorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
